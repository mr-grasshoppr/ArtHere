...
- **`arthere-mask.png`** — The Art Here logo as a CSS mask. White letterforms = opaque (show through), black = transparent. Source: `ArtHereLogo_052326_white.png`. Dimensions 3523×2690 (ratio 1.31). Layout: ART on top row (A | RT), HERE on bottom row (HE | RE).
...
- **`ArtHere_favicon.png`** — The "A" triangle favicon. Source: `ArtHereFavicon_052326.png`. Referenced by every page.
- **`arthere-mask-left.png`** / **`arthere-mask-right.png`** — The two halves of the logo (A/HE and RT/RE), each cropped square, used as masks for the two logo squares on the Portland grid. Sources: `ArtHereLogo_052326_leftside.png` / `_rightside.png`.
